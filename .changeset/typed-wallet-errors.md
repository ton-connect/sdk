...
When an injected wallet rejects a request instead of returning a TON Connect error response, a known wallet error code in the rejection (for example `new Error(300)`) is read as that error, so `sendTransaction` throws `UserRejectsError` and `walletError.normalized` is set. Any other rejection, a failure to send the request at all, and an answer that is neither a TON Connect response (an object with `result` or `error`) nor, for `connect`, a `connect` or `connect_error` event becomes the new `WalletTransportError` with the original value in `cause`; `sendTransaction` and `signData` report it as a signing failure. An injected `disconnect()` now always settles.
