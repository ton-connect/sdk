...
import { installPreviewWalletsCache } from '../utils/preview-wallets-cache';
import {
...
function PreviewModalFrame({
    ...
    previewSurface?: PreviewSurface;
    previewTrigger?: PreviewTrigger;
    /** Called when the user clicks inside the preview iframe. */
    onInteract?: () => void;
}) {
    ...
    useEffect(() => {
        const onMessage = (event: MessageEvent) => {
            ...
            if (event.data?.type === 'widget-builder-preview-ready') {
                readySignatureRef.current = previewSignatureRef.current;
                setPreviewReady(true);
            }

            if (event.data?.type === 'widget-builder-preview-pointerdown') {
...
function ModalPreviewPanel({
    ...
    const bringBlockToFront = useCallback(
        (id: string) => {
            ...
            // Already on top — skip the state update so repeated clicks and
            // drag starts don't re-render every preview iframe.
            if (topBlockId === id) {
                return;
            }

            stackOrderRef.current.set(id, maxOrder + 1);
    ...
    useEffect(() => {
        ...
        const onPointerMove = (event: PointerEvent) => {
            ...
            // Throttle transform updates to one per frame — pointermove can
            ...
            dragFrameRef.current = requestAnimationFrame(() => {
                ...
                updateDragVisual(dragState, x, y);
            });
        };
        ...
        <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden bg-background">
            ...
            >
                <div
                    ...
                    className={cn(
                        'relative',
                        isCompactLayout ? 'min-h-full' : 'h-full',
                        // Keep preview iframes out of pointer hit-testing while
                        // dragging so the captured pointer moves stay cheap.
                        draggingId && '[&_iframe]:pointer-events-none'
                    )}
                    style={{
                            ...
                            >
                                ...
                                >
                                    <PreviewModalFrame
                                        ...
                                        onInteract={() => bringBlockToFront(block.id)}
                                    />
                                </div>
                            </div>
                        );
...
export function WidgetBuilder() {
    ...
    // Share one wallets-list + icons load across all preview iframes.
    useEffect(() => {
        installPreviewWalletsCache();
    }, []);
